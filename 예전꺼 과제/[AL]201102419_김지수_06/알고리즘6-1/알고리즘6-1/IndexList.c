...
void IndexList_qsort (IndexList * _this, int left, int right);//index[left] 부터 index[right] 까지의 LectureID들을 quick sort 한다.
...
//index[i]가 가리키는 list의 LectureID와 index[j]가 가리키는 list의 LectureID를 비교한다. 실제로는 앞의 LectureID 값에서 뒤의 item 값을 뺀 값을 return 한다.
//일반적으로, index[p]가 가리키는 list의 LectureID:
//ArrayList_LectureIDAtIndex(indexList->list, indexList->index[p]);
...
IndexList * IndexList_newBySortingIndex (ArrayList * list)
{
	...
	IndexList * _this = NewObject (IndexList) ;
	_this->list = list ;
	_this->index = NewVector(LectureID, ArrayList_length(list)) ;
...
{
	// index[left] 부터 index[right] 까지의 LectureID들을 quick sort 한다.
...
int IndexList_compare (IndexList * _this, int i, int j)
{
	...
	// ArrayList_LectureIDAtIndex(_this->list, _this->index[i])
	// _this->index[j]가 가리키는 원소:
	// ArrayList_LectureIDAtIndex(_this->list, _this->index[j])
	// 이 두 원소를 비교한다.
	// 여기서는, 앞의 LectureID 값에서 뒤의 LectureID 값을 뺀 값을 return 한다.
	return (ArrayList_elementAtIndex(_this->list, (_this->index[i])).start)-(ArrayList_elementAtIndex(_this->list, (_this->index[j])).start)  ;
}
